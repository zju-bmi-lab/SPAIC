...
class NullEncoder(Encoder):
    ...
    def __init__(self, shape=None, num=None, dec_target=None, dt=None, coding_method='null', coding_var_name='O',
                 node_type=('excitatory', 'inhibitory', 'pyramidal', '...'), **kwargs):
    ...
    def torch_coding(self, source, device):
        ...
        if '[2]' in self.coding_var_name:
            for i in range(source.shape[0]):
                ...
                delta_times = torch.tensor((np.ceil(spike_index) - spike_index) * self.dt, device=device,
                                           dtype=self._backend.data_type)
...
class PoissonEncoding(Encoder):
    ...
    def torch_coding(self, source, device):
        ...
        if self.single_test:
            spikes = torch.rand([self.time_step, 1, self.shape[-1]], device=device).le(
                source * self.unit_conversion * self.dt)
    ...
    def next_stage(self):
        ...
        if self.end_time is not None and self.time > self.end_time:
            ...
            return torch.rand(self.source.shape, device=self.device).le(
                self.source * self.unit_conversion * self.dt).type(self._backend.data_type)
...
class PoissonEncoding1(Encoder):
    ...
    def __init__(self, shape=None, num=None, dec_target=None, dt=None, coding_method='poisson',
                 coding_var_name='O', node_type=('excitatory', 'inhibitory', 'pyramidal', '...'), **kwargs):
        super(PoissonEncoding1, self).__init__(shape, num, dec_target, dt, coding_method, coding_var_name, node_type,
                                               **kwargs)
...
class Relative_Latency(Encoder):
    ...
    def __init__(self, shape=None, num=None, dec_target=None, dt=None, coding_method=('poisson', 'spike_counts', '...'),
                 coding_var_name='O', node_type=('excitatory', 'inhibitory', 'pyramidal', '...'), **kwargs):
        super(Relative_Latency, self).__init__(shape, num, dec_target, dt, coding_method, coding_var_name, node_type,
                                               **kwargs)
...
class Constant_Current(Encoder):
    def __init__(self, shape=None, num=None, dec_target=None, dt=None, coding_method=('poisson', 'spike_counts', '...'),
                 coding_var_name='O', node_type=('excitatory', 'inhibitory', 'pyramidal', '...'), **kwargs):
        super(Constant_Current, self).__init__(shape, num, dec_target, dt, coding_method, coding_var_name, node_type,
                                               **kwargs)
...
class UniformEncoding(Encoder):
    ...
    def __init__(self, shape=None, num=None, dec_target=None, dt=None, coding_method=('poisson', 'spike_counts', '...'),
                 coding_var_name='O', node_type=('excitatory', 'inhibitory', 'pyramidal', '...'), **kwargs):
        super(UniformEncoding, self).__init__(shape, num, dec_target, dt, coding_method, coding_var_name, node_type,
                                              **kwargs)
    ...
    def torch_coding(self, source, device):
        ...
        period_timestep = torch.ceil(
            self.amp * max_time * (self.bias + amax - source) / ((self.bias + amax - amin) * self.dt)).unsqueeze(0)
        period_timestep = period_timestep + source.eq(amin) * 10 * max_time / self.dt
        time_step = torch.arange(self.time_step, device=device).expand((source_size, self.time_step)).t().view(
            spk_shape) + 1

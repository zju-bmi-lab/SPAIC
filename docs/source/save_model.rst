...
将会将完整的网络结构以及权重信息分别存储下来，该方式在使用时需要一个文件名filename，然后平台会在用户提供的目录或是默认的当前目录下新\
建'NetData/dir_name/dir_name.json'用于保存网络结构，权重的存储路径与 :code:`net.save_state` 相同，都会在目标目录下新建NetData文件夹，然后存于\
